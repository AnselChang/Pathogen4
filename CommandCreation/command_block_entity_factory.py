...
class CommandBlockEntityFactory:
    ...
    def create(self, path, adapter: PathAdapter) -> CommandBlockEntity:
        if adapter.type == CommandType.CUSTOM:
            return CustomCommandBlockEntity(path, adapter, self.database, self.entities, self.interactor, self.expansion, self.images, self.dimensions)
        else:
            return CommandBlockEntity(path, adapter, self.database, self.entities, self.interactor, self.expansion, self.images, self.dimensions)